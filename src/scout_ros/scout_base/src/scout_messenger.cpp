...
//      bms_status.Warning_Status_2 = state.Warning_Status_2;
  ...
  // Fill pose covariance (6x6 row-major): x,y,z,roll,pitch,yaw
  for (int i = 0; i < 36; ++i) odom_msg.pose.covariance[i] = 0.0;
  odom_msg.pose.covariance[0] = 0.05;    // var(x)
  odom_msg.pose.covariance[7] = 0.05;    // var(y)
  odom_msg.pose.covariance[14] = 0.0001; // var(z)
  odom_msg.pose.covariance[21] = 0.0001; // var(roll)
  odom_msg.pose.covariance[28] = 0.0001; // var(pitch)
  odom_msg.pose.covariance[35] = 0.1;    // var(yaw)

  odom_msg.twist.twist.linear.x = linear_speed_;
  odom_msg.twist.twist.linear.y = 0.0;
  odom_msg.twist.twist.angular.z = angular_speed_;

  // Fill twist covariance (6x6)
  for (int i = 0; i < 36; ++i) odom_msg.twist.covariance[i] = 0.0;
  odom_msg.twist.covariance[0] = 0.1;    // var(linear x)
  odom_msg.twist.covariance[7] = 0.1;    // var(linear y)
  odom_msg.twist.covariance[35] = 0.2;   // var(angular z)
