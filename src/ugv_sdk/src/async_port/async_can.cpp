...
void AsyncCAN::StopService() {
  // stop io thread
  io_context_.stop();
  // StopService() may be invoked from within the io thread (e.g. inside async
  // callback). Joining the current thread will throw std::system_error with
  // "Resource deadlock avoided". Avoid self-join.
  if (io_thread_.joinable()) {
    if (std::this_thread::get_id() != io_thread_.get_id()) {
      io_thread_.join();
    } else {
      // Detach so resources can be reclaimed; io_context_ is already stopped.
      io_thread_.detach();
    }
  }
  io_context_.reset();
